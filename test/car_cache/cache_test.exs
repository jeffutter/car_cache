defmodule CarCache.CacheTest do
  use ExUnit.Case
  ...
  def command_gen(%{cache: cache, inserted: inserted}) do
    ...
    frequency([
      ...
      {1, {:get, [cache, key]}},
      {1, {:delete, [cache, weighted_union([{4, known_key}, {1, key()}])]}}
    ])
  end

  defcommand :create_cache do
    def impl(name, max_size) do
      Cache.new(name, max_size: max_size)
    end
  ...
  defcommand :delete do
    ...
    def next(state, [_cache, key], cache) do
      ...
      %{state | cache: cache, inserted: inserted, full: full}
    end
  end
